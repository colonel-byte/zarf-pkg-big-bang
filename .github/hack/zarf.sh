#!/usr/bin/env bash

set -u
set -o pipefail

export BB_VERSION="3.2.0"

rm -rf   .direnv/bigbang
mkdir -p .direnv/bigbang/charts

curl -s -L -o ./.direnv/bigbang/oci_package_list.txt https://umbrella-bigbang-releases.s3.us-gov-west-1.amazonaws.com/umbrella/$BB_VERSION/oci_package_list.txt

yq -i '.components = [{"name": "bigbang-charts", "required": true, "images": []}]' zarf.yaml

for chart in $(cat .direnv/bigbang/oci_package_list.txt); do
  export yq_chart=$(printf '.components[0].images += "%s"' "$chart")
  echo "::debug::yq_chart='${yq_chart}'"
  yq -i "$yq_chart" zarf.yaml
done

for pkg in $(yq '.components[0].images[]' zarf.yaml | sort | uniq); do
  export chart_name=$(echo $pkg | sed -E 's#registry1.dso.mil/bigbang/##g; s#:.+##g')
  echo "::debug::chart_name='${chart_name}'"

  export chart_version=$(echo $pkg | sed -E 's#.+:##g')
  echo "::debug::chart_version='${chart_version}'"

  if [ "$chart_name" = "bigbang" ] || [ "$chart_name" = "istio-crds" ]; then
    continue
  fi

  rm -rf .direnv/bigbang/charts/$chart_name.yaml
  helm show chart oci://registry1.dso.mil/bigbang/$chart_name --version $chart_version 2>/dev/null >> .direnv/bigbang/charts/$chart_name.yaml

  export image_check=$(yq e '.annotations | has("images")' .direnv/bigbang/charts/$chart_name.yaml)
  export annotations="helm.sh/images"

  if [ $image_check == "true" ]; then
    export annotations="images"
  fi

  export yq_images=$(printf '(select(fi == 1) | .annotations."%s" | from_yaml | [.[].image] | sort) as $img' "$annotations")
  echo "::debug::yq_images='${yq_images}'"

  export yq_update=$(printf '(select(fi == 0) | .components += ([{"name": "%s", "x-version": "%s", "required": true, "images": $img}]))' "$chart_name" "$chart_version")
  echo "::debug::yq_update='${yq_update}'"

  yq ea -i "$yq_images | $yq_update" zarf.yaml .direnv/bigbang/charts/$chart_name.yaml
done
